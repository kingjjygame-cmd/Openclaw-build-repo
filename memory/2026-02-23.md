...
- **2026-02-24 01:40(UTC) 즉시 조치:**
  - 동작 정체의 근본 원인을 확정: 자동 전달 루프는 실행되었으나, 로컬 오케스트레이션이 의존하는 `GITHUB_TOKEN/SMTP_HOST/SMTP_USER/SMTP_PASSWORD` 설정이 세션에서 비어 있어 메일 전달로 못 넘어갔음.
  - 실수 재발 방지를 위해 `scripts/check_delivery_env.py` 추가:
    - 필수 env 사전 점검 + SMTP 인증 테스트 + 실패 원인 즉시 리포트.
  - 지금 상태에서 “빌드 성공+검증”은 계속 확인 가능하지만, 메일은 SMTP 자격정보가 맞아야만 완료.

- **2026-02-24 01:40(UTC) Gmail 직접 전송 방식 전환 완료(오케스트레이터):**
  - 사용자 요청에 따라 `SMTP` 의존 제거하고 `gog` CLI 기반 Gmail 직접 첨부 발송으로 정비.
  - 변경 항목:
    - `scripts/ci_build_and_deliver.py`: Gmail 발송 함수를 `gog gmail send --attach <apk>`로 교체, `GOG_ACCOUNT` 사용.
    - `scripts/check_delivery_env.py`: `GPG/SMTP` 체크 대신 `gog` 존재 여부 + `gog auth list --check` + `GOG_KEYRING_PASSWORD` 존재/오류 안내로 교체.
  - 목표: “빨리 돌고 성공 시 king 메일로 APK 첨부 발송”을 `gog` 인증 체계로 통합.
  - 현재 상태: `GOG_KEYRING_PASSWORD` 미입력 또는 토큰 키 해제 오류 시(`aes.KeyUnwrap` 등) 인증 실패 가능성 있음 → `GOG_KEYRING_PASSWORD` 확인 후 재시도 필요.
