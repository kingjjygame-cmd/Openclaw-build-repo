...
## 오픈클로 비서 운영 모드 (정기 모니터)
- 기본 모드: 매 세션 시작/요청 시점에 **쓰레드·커뮤니티·블로그·구글 검색 기반 동향 스캔 수행 여부를 점검**.
- 스캔 소스: 공식 포럼, 커뮤니티(클리앙/관련 게시판), 블로그, 검색(뉴스/웹), 릴리즈 노트 성격 공개 정보.
- 매일 1회 **요약형 패스**를 기준으로 하되, 사용자 요청 시 즉시 **긴급 수동 스캔** 수행.
- 보고 규칙:
  - 유의미 이슈 있으면 `오픈클로 비서`가 요약본(핵심 요지 3줄 + 출처 링크) 생성
  - 이슈 없으면 `현재 보고할 항목 없음` 상태로 기록
- 보안 이슈(취약점 경고/침해 징후)는 즉시 최우선 보고
- 쇼핑/로컬 작업과 분리하여 비동기화로 실행
...
- 명시 호출 우선:
  ...
  - "오픈클로 비서"
  - "앱개발 PM" 또는 "app-pm"
  - "앱개발 기획" 또는 "app-planner"
  - "앱개발 개발" 또는 "app-dev"
  - "앱개발 QA" 또는 "app-qa"

## 에이전트 세션 전환 규칙
- 하위 에이전트(정보검색/전문가/오픈클로 비서)로 명시 호출 후 **15분 이상 응답/대화가 없으면 자동으로 모카(총괄)로 복귀**한다.
- 복귀 시 기본 흐름은 모카의 운영 루틴(보고 여부 판단 → 요약/상세 분기)으로 재개한다.
